...
def test_liftover():
    ...
    The test data was prepared as follows:
        ...
        * We then picked positions from the genome as follows:
            For each interval we picked the first, the last, the 
            first-1, last+1, and first+4 positions.
            
            From the resulting ~40k points we chose 10000 random sites.
            We converted those via UCSC-hosted Web "liftOver" tool and 
...
def test_issue_2_3_4():
    ...
    NB: We are using the "live" hg38ToHg19.over.chain.gz file, hence if 
    it happens to change later on,
    the test may start failing. Just in case we have the original cached
    in the data directory as well.
    '''
    ...
    test_input = dict([
        (ln[3], (ln[0], int(ln[1]), ln[5].strip()))
        for ln in [line.split() for line in open(test_input)]])
    test_output = dict([
        (ln[3], (ln[0], int(ln[1]), ln[5].strip())) 
        for ln in [line.split() for line in open(test_output)]])
